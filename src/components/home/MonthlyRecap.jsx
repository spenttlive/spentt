...
export default function MonthlyRecap({ expenses, showToast, currency }) {
  ...
  if (!recap || dayOfMonth > 7) return null
  ...
  return (
    <div className="monthly-recap-wrap">
      ...
      <div className="mr-teaser" onClick={() => setExpanded(!expanded)}>
        <div className="mr-teaser-left">
          <span className="mr-teaser-emoji">🗓</span>
          <div>
            <div className="mr-teaser-title">{recap.monthName} recap</div>
            <div className="mr-teaser-sub">{sym}{recap.total.toLocaleString()} · {recap.verdict.personality}</div>
          </div>
        </div>
        <div className="mr-teaser-arrow">{expanded ? '↑' : '↓'}</div>
      </div>
